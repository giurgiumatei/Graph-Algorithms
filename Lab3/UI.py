...
class UI:
    ...
    def print_menu(self): # prints the menu
        ...
        print("13.Find out the lowest cost path from node A to node B: ")
    ...
    def option13(self):
        ...
        if source_point not in vertices or destination_point not in vertices:
            ...
            result=self.graph.Dijkstra(destination_point,source_point) #backwards search
            if result!=None:
                print(result[0])
                print("Cost is "+str(result[1]))
            else:
