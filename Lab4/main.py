    ...
    for i in range (0,verticex):
        ...
        if cp>0:
            ...
            while nr_of_edges>0:
                ...
                outbound[copy.deepcopy(j)].append([copy.deepcopy(i),copy.deepcopy(weight)])
                nr_of_edges-=1

    
    
    graph=Directed_Graphs(outbound,vertices,verticex,edges)
...
def graph_from_text_file():# function that reads a graph from a text file
    ...
    with open(r'C:\Users\giurg\OneDrive\Facultate\Grafuri\Lab1\graph_lab4_1.txt','r') as f: #change path here if you want to read from a different text file
        reader=f.readline()
        reader=reader.strip("\n")
        line=reader.split(' ')
        ...
        outbound[copy.deepcopy(line[1])].append([copy.deepcopy(line[0]),copy.deepcopy(line[2])])
    ...
    graph=Directed_Graphs(outbound,vertices,nr_of_vertices,nr_of_edges)
